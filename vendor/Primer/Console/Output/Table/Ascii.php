...
namespace Primer\Console\Output\Table;

use Primer\Console\Colors;
...
class Ascii extends Renderer
{
    ...
    public function setWidths(array $widths)
    {
        if (is_null($this->_constraintWidth)) {
            $this->_constraintWidth = (int)$this->columns();
        }
        ...
        $col_padding_count
            = $col_count * strlen($this->_characters['padding']) * 2;
        $max_width
            =
            $this->_constraintWidth - $col_borders_count - $table_borders_count
            - $col_padding_count;
    ...
    public function border()
    {
        if (!isset($this->_border)) {
            ...
            foreach ($this->_widths as $width) {
                $this->_border .= str_repeat(
                    $this->_characters['line'], $width + 2
                );
                $this->_border .= $this->_characters['corner'];
            }
        }
        ...
        if (count($row) > 0) {
            ...
            foreach ($row as $col => $value) {
                ...
                if ($original_val_width > $col_width) {
                    ...
                    $value = \cli\safe_substr(
                        $value, $col_width, $original_val_width
                    );
                    $i = 0;
                    do {
        ...
        if ($extra_row_count) {
            ...
            do {
                ...
                $row_values = array_map(
                    array($this, 'padColumn'), $row_values,
                ...
                $ret .= PHP_EOL . join(
                        $this->_characters['border'], $row_values
                    );
            } while ($has_more);
        }
        return $ret;
    ...
    {
        return $this->_characters['padding'] . Colors::pad(
            $content, $this->_widths[$column]
        ) . $this->_characters['padding'];
    }
}
